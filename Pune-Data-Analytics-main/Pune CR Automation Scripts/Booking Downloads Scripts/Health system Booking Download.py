...
data = pd.read_excel(r'C:\Users\Vinayak.Laxmeshwar\Documents\Files\BD\HealthSystem_client.xlsx')
...
for unique_id, child_unique_id, name in zip(unique_ids_parent, unique_ids_child, client_name):
    ...
    if child_unique_id_str == '':
        sql_query_parent_template = f"""SELECT distinct a.appointment_id,
                        ...
                        --and a.MONOLITH_PROVIDER_ID in('77426')
                        ----If the parent ID is filled out, use this line
                        AND a.parent_strategic_id_with_current_mapping in ('{unique_id_str}')
                        --If the regular ID is filled out or if you need to add multiple IDs, use this line
                        --AND a.monolith_strategic_id_with_current_mapping IN ('283')
        ...
        directory = r'C:\Users\Vinayak.Laxmeshwar\Documents\HealthSystem Booking Downloads'
        filename = os.path.join(directory, f"{name_str} Booking Download - {Last_month}.csv")
                ##for row in resultList:
        df.to_csv(filename, index=False,header=True)
    else:
        sql_query_child_template = f"""SELECT distinct a.appointment_id,
                           a.monolith_parent_strategic_id,
                           a.parent_strategic_name,
                           a.provider_npi as Provider_NPI
                            ,monolith_professional_id
                        ...
                        --and a.MONOLITH_PROVIDER_ID in('77426')
                        ----If the parent ID is filled out, use this line
                        --AND a.parent_strategic_id_with_current_mapping in ()
                        --If the regular ID is filled out or if you need to add multiple IDs, use this line
...
subject = 'Health System Bookings Download- ' + Last_month + '.'
message = """Hi All,

The Health System Client Booking Downloads has ran successfully.
